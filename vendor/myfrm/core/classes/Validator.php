...
class Validator
{
    ...
    public $rules_list = ['required', 'min', 'max', 'email', 'match', 'unique'];
    protected $messages = [
        ...
        'match' => 'The :fieldname: field must be match :rulevalue:',
    ...
    protected function check($field)
    {
        foreach ($field['rules'] as $rule => $rule_value) {
            if (in_array($rule, $this->rules_list)) {
                if (!call_user_func_array([$this, $rule], [$field['value'], $rule_value])) { //коллабал функция передвая массив аргументов 
                    $this->addError(
                        $field['fieldname'],
                        str_replace(
                            [':fieldname:', ':rulevalue:'],
